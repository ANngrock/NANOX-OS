...
int nx_elf_plan(const uint8_t *img, uint64_t size, struct nx_elf_plan *plan)
{
    ...
    for (uint16_t i = 0; i < phnum; i++) {
        ...
        if (vaddr != paddr || paddr > UINT64_MAX - memsz || paddr < NX_KERNEL_LOAD_MIN ||
            paddr + memsz > NX_KERNEL_LOAD_MAX)
            return NX_ELF_E_SEG_ADDR;
        if (plan->segment_count && paddr < prev_end)
    ...
    for (uint32_t i = 0; i < plan->segment_count; i++) {
        ...
        if ((s->flags & PF_X) && entry >= s->phys_addr && entry - s->phys_addr < s->mem_size)
            entry_ok = 1;
    }
    if (!entry_ok)
        return NX_ELF_E_ENTRY;

    /* prev_end <= NX_KERNEL_LOAD_MAX, so rounding up cannot overflow. */
    plan->entry = entry;
    plan->span_base = plan->segments[0].phys_addr & ~PAGE_MASK;
    plan->span_end = (prev_end + PAGE_MASK) & ~PAGE_MASK;
    if (plan->span_end - plan->span_base > NX_KERNEL_SPAN_MAX)
...
const char *nx_elf_strerror(int err)
{
    ...
    case NX_ELF_E_SPAN: return "kernel span too large";
