...
(С M2 раскладка другая: пользовательские сегменты `0x18`/`0x20`, TSS `0x28`,
`RSP0` задачи, шлюз `0x80` с DPL 3 — [m2-kernel.md §3](m2-kernel.md#3-gdt-tss-idt).)
...
появления пользовательских адресных пространств (M2). Пересмотрено в M2:
ядро остаётся в слоте 0 PML4, пользовательская половина — слоты 1–255
([m2-kernel.md §4](m2-kernel.md#4-адресное-пространство-задачи)).
...
- После проверки таймер маскируется и прерывания запрещаются: до
  планировщика (M2) периодические прерывания не нужны. В режимах M2
  планировщик снова включает периодический режим с той же калибровкой.
...
--count 3` (раздел 1; с M2 в тот же вызов добавлен `m2-sched`); результат —
`out/runs/<время>-repeat-<сценарий>/repeat.json`.
