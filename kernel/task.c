...
{
    if (rbp < NX_KSTACK_BASE || rbp >= NX_KSTACK_BASE + NX_TASK_MAX * NX_KSTACK_STRIDE)
        return 0;
...
static int load_elf(uint64_t root, const uint8_t *elf, uint64_t size, uint64_t *entry)
{
    ...
    const struct nx_elf_rules rules = {
        NX_USER_BASE, NX_USER_STACK_TOP - (NX_USER_STACK_PAGES + 1) * NX_PAGE_4K,
        64ull << 20, 0, 1};
    if (nx_elf_plan_rules(elf, size, &rules, &plan) != NX_ELF_OK)
        return NX_EINVAL;
...
static void print_log(const struct nx_task *t)
{
    uint32_t start = 0;
    for (uint32_t i = 0; i < t->log_len; i++) {
        if (t->log[i] != '\n' && i + 1 != t->log_len)
            continue;
        uint32_t end = t->log[i] == '\n' ? i : i + 1;
    ...
    if (t->log_truncated)
        nx_printf("NANOX: USER %s#%u: [log truncated]\n", t->name, t->id);
}
