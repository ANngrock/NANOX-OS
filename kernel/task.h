...
struct nx_task {
    ...
    uint32_t killer_id;
    /* Lines written with NX_SYS_DEBUG_WRITE, printed when the task is reaped. */
    char log[NX_TASK_LOG];
    uint32_t log_len;
    int log_truncated;
};
...
/* Waits until t is DEAD, then releases its resources and prints its log and
 * end record.  Returns the exit code (or -vector for faults). */
