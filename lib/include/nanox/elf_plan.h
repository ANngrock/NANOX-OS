...
/* Upper bound on the span of a user program (M2). */
#define NX_USER_SPAN_MAX (64ull << 20)
...
/* User program rules (M2): segments in [NX_USER_BASE, below the user stack),
 * span at most NX_USER_SPAN_MAX, no W+X segment, p_paddr ignored. */
int nx_elf_plan_user(const uint8_t *image, uint64_t size, struct nx_elf_plan *plan);
