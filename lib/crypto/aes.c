/*
 * AES (FIPS 197) encryption and AES-GCM (NIST SP 800-38D); see
 * nanox/aead.h.  Only the forward cipher is needed (GCM uses CTR mode).
 */
#include <nanox/aead.h>
#include <nanox/hash.h>
#include <nanox/string.h>

static const uint8_t SBOX[256] = {
    ,
};

static uint32_t sub_word(uint32_t w)
{
    return (uint32_t)SBOX[w >> 24] << 24 | (uint32_t)SBOX[(w >> 16) & 0xFF] << 16 |
           (uint32_t)SBOX[(w >> 8) & 0xFF] << 8 | SBOX[w & 0xFF];
}

void nx_aes_init(struct nx_aes *a, const uint8_t *key, uint32_t key_len)
{
    uint32_t nk = key_len / 4, total = 4 * (nk + 7);
    a->rounds = (int)nk + 6;
    for (uint32_t i = 0; i < nk; i++)
        a->rk[i] = (uint32_t)key[4 * i] << 24 | (uint32_t)key[4 * i + 1] << 16 |
                   (uint32_t)key[4 * i + 2] << 8 | key[4 * i + 3];
    uint32_t rcon = 0x01;
    for (uint32_t i = nk; i < total; i++) {
        uint32_t t = a->rk[i - 1];
        if (i % nk == 0) {
            t = sub_word(t << 8 | t >> 24) ^ (rcon << 24);
            rcon = (rcon << 1) ^ ((rcon >> 7) * 0x11Bu);
        } else if (nk > 6 && i % nk == 4) {
            t = sub_word(t);
        }
        a->rk[i] = a->rk[i - nk] ^ t;
    }
}

static uint8_t xtime(uint8_t x)
{
    return (uint8_t)((x << 1) ^ (((x >> 7) & 1u) * 0x1Bu));
}

void nx_aes_encrypt(const struct nx_aes *a, const uint8_t in[16], uint8_t out[16])
{
    uint8_t s[16];
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            s[4 * c + r] = in[4 * c + r] ^ (uint8_t)(a->rk[c] >> (24 - 8 * r));
    for (int round = 1; round <= a->rounds; round++) {
        uint8_t t[16];
        for (int i = 0; i < 16; i++)
            t[i] = SBOX[s[i]];
        /* ShiftRows: row r of column c comes from column c + r */
        for (int c = 0; c < 4; c++)
            for (int r = 0; r < 4; r++)
                s[4 * c + r] = t[4 * ((c + r) % 4) + r];
        if (round != a->rounds) {
            for (int c = 0; c < 4; c++) { /* MixColumns */
                uint8_t *col = s + 4 * c;
                uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
                uint8_t all = a0 ^ a1 ^ a2 ^ a3;
                col[0] ^= all ^ xtime(a0 ^ a1);
                col[1] ^= all ^ xtime(a1 ^ a2);
                col[2] ^= all ^ xtime(a2 ^ a3);
                col[3] ^= all ^ xtime(a3 ^ a0);
            }
        }
        for (int c = 0; c < 4; c++)
            for (int r = 0; r < 4; r++)
                s[4 * c + r] ^= (uint8_t)(a->rk[4 * round + c] >> (24 - 8 * r));
    }
    memcpy(out, s, 16);
}

/* ---- GCM ---------------------------------------------------------------------- */

/* x = x * y in GF(2^128) (bit-reflected, SP 800-38D §6.3), without
 * secret-dependent branches. */
static void gf_mul(uint8_t x[16], const uint8_t y[16])
{
    uint8_t z[16] = {0}, v[16];
    memcpy(v, y, 16);
    for (int i = 0; i < 128; i++) {
        uint8_t bit = (uint8_t)((x[i / 8] >> (7 - i % 8)) & 1u);
        uint8_t mask = (uint8_t)(0u - bit);
        for (int j = 0; j < 16; j++)
            z[j] ^= v[j] & mask;
        uint8_t lsb = v[15] & 1u;
        for (int j = 15; j > 0; j--)
            v[j] = (uint8_t)(v[j] >> 1 | v[j - 1] << 7);
        v[0] >>= 1;
        v[0] ^= (uint8_t)(0xE1u & (0u - lsb));
    }
    memcpy(x, z, 16);
}

static void ghash_update(uint8_t y[16], const uint8_t h[16], const uint8_t *p, uint32_t len)
{
    while (len) {
        uint32_t n = len < 16 ? len : 16;
        for (uint32_t i = 0; i < n; i++)
            y[i] ^= p[i];
        gf_mul(y, h);
        p += n;
        len -= n;
    }
}

void nx_gcm_init(struct nx_gcm *g, const uint8_t *key, uint32_t key_len)
{
    static const uint8_t zero[16];
    nx_aes_init(&g->aes, key, key_len);
    nx_aes_encrypt(&g->aes, zero, g->h);
}

static void ctr_block(uint8_t ctr[16])
{
    for (int i = 15; i >= 12; i--)
        if (++ctr[i])
            break;
}

static void gcm_tag(const struct nx_gcm *g, const uint8_t nonce[12], const uint8_t *aad,
                    uint32_t aad_len, const uint8_t *ct, uint32_t len, uint8_t tag[16])
{
    uint8_t y[16] = {0}, lens[16] = {0}, j0[16], ek[16];
    ghash_update(y, g->h, aad, aad_len);
    ghash_update(y, g->h, ct, len);
    uint64_t abits = (uint64_t)aad_len * 8u, cbits = (uint64_t)len * 8u;
    for (int i = 0; i < 8; i++) {
        lens[7 - i] = (uint8_t)(abits >> (8 * i));
        lens[15 - i] = (uint8_t)(cbits >> (8 * i));
    }
    ghash_update(y, g->h, lens, 16);
    memcpy(j0, nonce, 12);
    j0[12] = j0[13] = j0[14] = 0;
    j0[15] = 1;
    nx_aes_encrypt(&g->aes, j0, ek);
    for (int i = 0; i < 16; i++)
        tag[i] = ek[i] ^ y[i];
}

static void gcm_ctr(const struct nx_gcm *g, const uint8_t nonce[12], const uint8_t *in,
                    uint32_t len, uint8_t *out)
{
    uint8_t ctr[16], ks[16];
    memcpy(ctr, nonce, 12);
    ctr[12] = ctr[13] = ctr[14] = 0;
    ctr[15] = 1;
    while (len) {
        ctr_block(ctr);
        nx_aes_encrypt(&g->aes, ctr, ks);
        uint32_t n = len < 16 ? len : 16;
        for (uint32_t i = 0; i < n; i++)
            out[i] = in[i] ^ ks[i];
        in += n;
        out += n;
        len -= n;
    }
}

void nx_gcm_seal(const struct nx_gcm *g, const uint8_t nonce[12], const uint8_t *aad,
                 uint32_t aad_len, const uint8_t *pt, uint32_t len, uint8_t *ct,
                 uint8_t tag[16])
{
    gcm_ctr(g, nonce, pt, len, ct);
    gcm_tag(g, nonce, aad, aad_len, ct, len, tag);
}

int nx_gcm_open(const struct nx_gcm *g, const uint8_t nonce[12], const uint8_t *aad,
                uint32_t aad_len, const uint8_t *ct, uint32_t len, const uint8_t tag[16],
                uint8_t *pt)
{
    uint8_t want[16];
    gcm_tag(g, nonce, aad, aad_len, ct, len, want);
    if (!nx_ct_equal(want, tag, 16)) {
        memset(pt, 0, len);
        return -1;
    }
    gcm_ctr(g, nonce, ct, len, pt);
    return 0;
}
