...
Пункты ниже ещё не выполнены. Переход к следующему этапу подтверждается артефактами и проверками. Работы над исследовательской веткой не заменяют обязательные системные этапы.

### M0. Воспроизводимый стенд

- [ ] Выбрать и записать точную QEMU/UEFI-конфигурацию.
- [ ] Проверить доступные инструменты и закрепить версии toolchain.
- [ ] Создать сборку образа, запуск и headless test harness.
- [ ] Записывать command line, исходную ревизию, хеш образа, лог и exit status.
- [ ] Документировать интерфейс boot info, структуру памяти и правила ошибок.

Готовность: новый checkout можно собрать и испытать одной документированной последовательностью без ручного редактирования образа.
...
- [ ] Пользовательский код запускается в отдельном адресном пространстве.
- [ ] Несколько задач переключаются вытесняющим scheduler.
- [ ] IPC передаёт сообщение и handle с определёнными правами.
- [ ] Некорректный syscall не повреждает ядро.
- [ ] Обычное приложение не читает память другого приложения.
- [ ] Sovereign выполняет предусмотренную привилегированную операцию над тестовой задачей.

Готовность: проверены и граница изоляции обычных приложений, и полный системный статус Core.
