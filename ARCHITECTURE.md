...
- [x] Работают NCI, task engine и события состояния. Исполнитель `bin/core` принимает запросы NCI v1 и проводит каждый через жизненный цикл §8.2; ядро ведёт журнал событий задач — сценарий `m3-nci`, отрицательный контроль `m3-events-off`, host-тесты разбора NCI, task engine и журнала — [docs/m3-core.md](docs/m3-core.md#1-критерии-m3-и-их-проверка).
- [x] Подключён host bridge с адаптером модели и явно описанной границей guest/host. Канал — COM2 и unix-сокет host, граница — [docs/m3-core.md §2](docs/m3-core.md#2-граница-guesthost); сценарии `m3-agent` (адаптер `mock`) и `m3-model-down` (адаптер `unavailable`). Адаптер настоящего provider написан, но не проверен.
- [ ] ИИ по запросу перечисляет задачи, запускает тестовую нагрузку, измеряет и останавливает её — цепочка выполняется ядром и проверена сценарием `m3-agent`, но с мок-моделью, а не ИИ; с настоящей моделью не проверено ([docs/m3-core.md §12](docs/m3-core.md#12-что-не-проверено)).
- [x] Исполнитель проверяет конечное состояние, а не только успешный ответ функции. Фаза `VERIFYING` сверяет состояние задачи и события ядра — сценарий `m3-agent`, отрицательный контроль `m3-kill-noop` (ядро сообщает успех без эффекта, исполнитель отвечает `VERIFY_FAILED`).
- [x] Проверены старые handles, повтор запроса, потеря ответа и недоступность модели — сценарии `m3-faults`, `m3-model-down`, отрицательный контроль `m3-nodedup`, host-тесты task engine и восстановления — [docs/m3-core.md §8](docs/m3-core.md#8-сценарии-стенда).
- [x] Сохранена трасса: запрос, выбранное действие, вызов, результат и проверка. `bridge-trace.jsonl` в записи запуска и трасса исполнителя в serial-логе; полнота трассы проверяется в `m3-agent` и host-тестами — [docs/m3-core.md §7](docs/m3-core.md#7-host-bridge).

Готовность: действия реально выполняются внутри собственного ядра. Мок модели годится для теста протокола, но демонстрация ИИ требует подключения настоящего provider. Пока не подтверждена: действия выполняет ядро NANOX, критерии 1, 2, 4–6 проверены в QEMU (TCG) сценариями `make test` с мок-моделью; настоящий provider не подключался.

### M4. Постоянное состояние

- [x] Реализованы virtio-blk, формат хранилища, версии объектов и commit. Драйвер virtio-blk (virtio 1.x по PCI, опрос) и системные вызовы 22–25 в ядре; хранилище nxstore v1 (`lib/store.c`: copy-on-write, два слота суперблока, корень на поколение, `oid`/`version` объектов, фиксация с двумя flush) в `bin/core`; независимая host-реализация формата `tools/store/nxstore.py` — сценарий `m4-blk`, host-тесты `test_store.c`, перекрёстные проверки двух реализаций `test_nxstore.py` — [docs/m4-store.md](docs/m4-store.md#1-критерии-m4-и-их-проверка).
- [x] Задачи, конфигурация и история сохраняются между загрузками. Конфигурация, записи task engine и журнал фиксаций переживают перезагрузку; повтор запроса прежней загрузки отвечается из записи, прерванное действие становится `OUTCOME_UNKNOWN` — двухзагрузочный сценарий `m4-persist`, отрицательный контроль `m4-persist-amnesia` — [docs/m4-store.md §8](docs/m4-store.md#8-состояние-исполнителя-bincore-в-хранилище).
- [x] Проверены оборванные записи, повреждённый корень, полный диск и восстановление. Серия сбоев `m4-crash`: остановка перед каждой из 60 операций записи и flush рабочей нагрузки с политиками потери незафиксированных записей (все, ни одной, последняя оборвана, только последняя) — 153 пары загрузок и точка после завершения, 0 нарушений; отрицательный контроль `m4-crash-noflush`; сценарии `m4-corrupt-root`, `m4-unmountable`, `m4-full`; host-симуляция сбоя перед каждой операцией с перебором исходов и пять отрицательных контролей — [docs/m4-store.md §10](docs/m4-store.md#10-проверка-устойчивости-к-сбоям). Сбой моделируется остановкой гостя и эмулируемым кэшем записи, а не настоящим отключением питания.
- [x] Определена политика удаления старых данных и ссылок на поколения. Окно из 4 поколений, до 4 закреплений поколений, журнал на 16 фиксаций, освобождение блоков только после устойчивой фиксации, резерв для освобождающих транзакций — [docs/m4-store.md §7](docs/m4-store.md#7-политика-хранения-и-удаления); сценарий `m4-retention`, host-тесты `test_retention`, `test_full`.

Готовность: после искусственного сбоя наблюдается допустимое целое состояние; изменения не объявляются сохранёнными до завершения обещанного протокола durability. Подтверждена в модели сбоя: в QEMU (TCG) после остановки в каждой точке протокола рабочей нагрузки восстанавливается поколение не старше объявленного сохранённым и не новее начатого, с содержимым ровно этого поколения — по исполнителю и по независимому host-читателю; с flush, который ничего не делает, объявленное сохранённым теряется. Настоящее отключение питания, кэш QEMU и хоста и реальное оборудование не проверялись ([docs/m4-store.md §15](docs/m4-store.md#15-что-не-проверено)).

### M5. Сеть и автономный provider

- [x] Реализованы сетевой драйвер, необходимые протоколы и диагностика. virtio-net (virtio 1.x по PCI, опрос) и системные вызовы 26–28 в ядре; стек Ethernet/ARP/IPv4/ICMP/UDP/DNS/TCP (`lib/net`) и операции диагностики `net.*` в `bin/core` — сценарии `m5-net` (включая link down/up через QMP), `m5-net-loss`, host-тесты `test_net.c` — [docs/m5-net.md](docs/m5-net.md#1-критерии-m5-и-их-проверка).
- [x] Работают entropy/CSPRNG, защищённое соединение, проверка сертификатов и конфигурация ключа. virtio-rng и HMAC-DRBG с тестами здоровья, свой клиент TLS 1.3 с проверкой цепочки, срока и имени (отключить нельзя), ключ, модель и якоря — объекты хранилища — сценарии `m5-tls` (против OpenSSL, три вида отказа проверки), `m5-nokey`, host-тесты `test_crypto.c`, `test_tls.py` — [docs/m5-net.md §6–8](docs/m5-net.md#6-энтропия-csprng-и-криптография).
- [x] Cognitive Core вызывает provider без host bridge. Цикл агента `agent.ask` в `bin/core` спрашивает модель по протоколу Messages через свой стек и TLS и выполняет её вызовы инструментов через task engine — сценарии `m5-agent`, `m5-agent-loss` (отрицательный контроль `m5-noretx`) — с локальным тестовым provider со сценарной политикой; с настоящим provider не проверено ([docs/m5-net.md §14](docs/m5-net.md#14-что-не-проверено)).
- [x] Проверены timeouts, обрыв потока ответа, повтор и восстановление соединения — сценарий `m5-faults` (ошибки HTTP с `Retry-After`, ошибка в потоке, обрыв TCP и ранний `close_notify` посреди потока, молчание сервера, RST, закрытие keep-alive, исчерпание попыток), отрицательный контроль `m5-noretry` — [docs/m5-net.md §8.3](docs/m5-net.md#83-повторы-и-тайм-ауты).
- [x] Потеря API оставляет доступной восстановительную консоль — сценарий `m5-console`: при недоступном provider и опущенном линке консоль COM2 наблюдает, запускает и останавливает задачи и показывает диагностику, после возврата provider агент работает снова.

Готовность: собственная ОС взаимодействует с моделью через свой сетевой стек; телеметрия различает сетевой сбой, ошибку provider и ошибку выполнения действия. Подтверждена с тестовым provider: сценарий `m5-classes` получает `NET_ERROR`, `PROVIDER_ERROR` и `ACTION_ERROR` у одного гостя в ответе и в телеметрии, отрицательный контроль `m5-flattel`; ответы даёт сценарная политика, а не языковая модель, и запрос к настоящему provider не выполнялся — в среде нет ключа ([docs/m5-net.md §1](docs/m5-net.md#1-критерии-m5-и-их-проверка)).
